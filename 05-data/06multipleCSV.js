...
//data source: https://data.cityofnewyork.us/City-Government/Borough-Boundaries-Water-Areas-Included-/tv64-9x69
 ...
 subwayEntrance = loadTable('/05-data/data/DOITT_SUBWAY_ENTRANCE_01_13SEPT2010.csv', 'csv', 'header');
...
function setup(){
	...
	parseMultipolygon(boundaries.getColumn(1)[0]);//1 without water, 2 with
	
	console.log("SUBWAY ENTRANCES");
	console.log(subwayEntrance);
	//boundariesStats = dataInfo(boundaries);
	//scale(2.5);
...
function dataView(range){
	...
	while(index<5){
		if(range == 'head'){
			...
			rIndex = boundaries.getRowCount()-(5-index);
		}
		console.log(boundaries.getRow(rIndex).arr);
		index++;
	}
}
...
function plot(){
	stroke(0);
	let shapes = boundaries.getColumn(1);//1 without water, 2 with
	for(let s =0;s< 5;s++){
